...
export default class Status extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    // Show rotation age for remote variables
    if (remote?.variables?.length > 0) {
      this.log("");
      this.log(`  ${chalk.dim("Secret Ages:")}`);
      for (const v of remote.variables.slice(0, 5)) {
        // Use updatedAt from the variable or environment createdAt as proxy
        const ageMs = Date.now() - (v.updatedAt ?? 0);
        const ageDays = Math.floor(ageMs / 86400000);

        // Skip variables with missing/zero updatedAt
        if (!v.updatedAt) continue;

        let ageStr = ageDays < 30
          ? `${ageDays} days`
          : ageDays < 365
            ? `${Math.floor(ageDays / 30)} months`
            : `${Math.floor(ageDays / 365)} years`;

        let ageColor = colors.success;
        let ageSymbol: string = symbols.success;

        if (ageDays > 365) {
          ageColor = colors.error;
          ageSymbol = symbols.error;
          ageStr += ' ⚠️  Rotate immediately';
        } else if (ageDays > 180) {
          ageColor = colors.warning;
          ageSymbol = symbols.warning;
          ageStr += ' — Consider rotating';
        }

        this.log(`  ${ageSymbol}  ${colors.key(v.key).padEnd(30)}  ${ageColor(ageStr)}`);
      }
      if (remote.variables.length > 5) {
        this.log(`    ${chalk.dim(`... and ${remote.variables.length - 5} more`)}`);
      }
    }
  }
}
