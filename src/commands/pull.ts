...
export default class Pull extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const exampleKeys = exampleLines
        .map(l => l.split('=')[0]?.trim())
        .filter(Boolean) as string[];

      const pulledKeys = new Set(result.variables.map((v: any) => v.key));
      const missingKeys = exampleKeys.filter(k => !pulledKeys.has(k));

      if (missingKeys.length > 0) {
        this.log(`  ${colors.warning("⚠")}  ${missingKeys.length} key${missingKeys.length === 1 ? '' : 's'} in .env.example not in remote:`);
        for (const k of missingKeys) {
          this.log(`    ${chalk.dim("·")}  ${colors.key(k)}`);
        }
        this.log(`  ${chalk.dim("→")}  These may need to be added with slickenv push`);
        this.log("");
      }
    } catch {
      // .env.example doesn't exist or unreadable — skip check
    }

    // Run linter on pulled content (warnings only, not blocking)
    const pulledVars = result.variables.map((v: any) => ({
      key: v.key, value: '', visibility: v.visibility ?? 'private',
      type: v.type ?? 'string', required: v.required ?? false,
      metadataWasInjected: false,
    }));
    const lintIssues = lintEnvFile(pulledVars);
    const lintErrors = lintIssues.filter(i => i.level === 'error');
    if (lintErrors.length > 0) {
      this.log(`  ${colors.warning("⚠")}  Linter: ${lintErrors.length} issue${lintErrors.length === 1 ? '' : 's'} in remote env`);
    }
