...
export default class Init extends BaseCommand {
  ...
  private async smartScan(dir: string): Promise<void> {
    ...
    if (newKeys.length > 0) {
      ...
      // Offer to write .env skeleton
      const shouldWrite = isTTY
        ? await this.promptYesNo(`Write a .env skeleton with these ${newKeys.length} key${newKeys.length === 1 ? '' : 's'}?`, true)
        : false;

      if (shouldWrite) {
        await this.writeEnvSkeleton(dir, newKeys, existingKeys);
      }
    }
  }

  private async writeEnvSkeleton(dir: string, newKeys: string[], existingKeys: Set<string>): Promise<void> {
    ...
    // Build skeleton lines for new keys only
    ...
    for (const key of newKeys) {
      ...
      const isPublic = key.startsWith('NEXT_PUBLIC_') || key.startsWith('VITE_') || key.startsWith('REACT_APP_');
      const comment = isPublic ? '# public — safe to expose to browser' : '# private — keep secret';
      skeletonLines.push(comment);
      skeletonLines.push(`${key}=`);
      addedCount++;
    }

    if (addedCount === 0) return;

    const combined = existingContent + skeletonLines.join('\n') + '\n';
    await writeFile(envPath, combined, 'utf8');

    this.log(`  ${colors.success(symbols.success)}  Wrote ${addedCount} key${addedCount === 1 ? '' : 's'} to ${chalk.bold('.env')} ${chalk.dim('(fill in the values before pushing)')}`);
    this.log(`  ${chalk.dim("→")}  Run ${colors.highlight("slickenv push")} once your .env is ready`);
  }
